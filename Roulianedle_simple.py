...
def hard_mode_yellow_check(guess, dict): # to run after green check (important). Now checking for if found yellows are present in guess
    ...
    for i, pos in enumerate(dict): # temp copy without green, helps with a double letter where 1 was found green and 1 found yellow.
        if dict[pos]["colour"] == "🟨":
            guess_without_greens[i] = guess[i]
            yellow_list.append(dict[pos]["letter"])
        elif dict[pos]["colour"] == "⬜️":
            guess_without_greens[i] = guess[i]
        else:
            pass
    for i in range(len(yellow_list)):
        if yellow_list[i] not in guess_without_greens:
            flag = False
            break
        else:
            pass
...
while try_number<8:
    ...
    if not hard_mode or try_number == 1:
        ...
        while guess_valid == False:
            guess = input("Enter your guess: ")
            if isinwordlist(guess) == False:
                print("Not in word list. Try again.")
            elif hard_mode_green_check(guess,previous_dict) == False:
                print("This is hard mode, respect (and eat) your greens!")
            elif hard_mode_yellow_check(guess, previous_dict) == False:
                print("You've forgotten to re-use the found yellows...")
            else:
                print("Guess validated, processing!")
                guess_valid = True
    ...
    if(guess==solution):
        print("🟩🟩🟩🟩🟩")
        if try_number == 1:
            print("WOW, did you hack Roulianedle or something? That's crazy luck!!")
        elif try_number == 5:
            print("Well done, a perfect 5/7 score. I couldn't be prouder of you!")
        else:
